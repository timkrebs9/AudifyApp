...
def test_user_login_wrong_email(client):
    response = client.post(
        "/auth/login", data={"username": "abc@describly.com", "password": USER_PASSWORD}
    )
    assert response.status_code == 400
...
def test_user_login_inactive_account(client, inactive_user):
    response = client.post(
        "/auth/login", data={"username": inactive_user.email, "password": USER_PASSWORD}
    )
    assert response.status_code == 400
...
def test_user_login_unverified(client, unverified_user):
    response = client.post(
        "/auth/login",
        data={"username": unverified_user.email, "password": USER_PASSWORD},
    )
    assert response.status_code == 400
