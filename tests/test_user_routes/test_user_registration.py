...
def test_create_user_with_invalid_email(client):
    data = {"name": "Test User", "email": "keshari.com", "password": USER_PASSWORD}
    response = client.post("/users/", json=data)
    assert response.status_code != 201


def test_create_user_with_empty_password(client):
    data = {"name": "Test User", "email": USER_EMAIL, "password": ""}
    response = client.post("/users/", json=data)
    assert response.status_code != 201


def test_create_user_with_numeric_password(client):
    data = {"name": "Test User", "email": USER_EMAIL, "password": "1232382318763"}
    response = client.post("/users/", json=data)
    assert response.status_code != 201


def test_create_user_with_char_password(client):
    data = {"name": "Test User", "email": USER_EMAIL, "password": "asjhgahAdF"}
    response = client.post("/users/", json=data)
    assert response.status_code != 201


def test_create_user_with_alphanumeric_password(client):
    data = {"name": "Test User", "email": USER_EMAIL, "password": "sjdgajhGG27862"}
    response = client.post("/users/", json=data)
    assert response.status_code != 201
